...
def generate_response_template(weather):
    response_template = ""
    response_template += f'{weather["description"]}.\n'
...
def get_coordinates_by_city_name(name,message_in):
    ...
    # Если найдено хотя бы одно соответствующее место, возвращаем его координаты
    if len(locations) > 0:
...
def get_weather_by_coordinates(latitude, longitude):
    try:
        # Формирование URL-адреса запроса к OpenWeatherMap API для получения погоды по координатам
        url = f"http://api.openweathermap.org/data/2.5/weather?lat={latitude}&lon={longitude}&appid={OPEN_WEATHER_MAP_TOKEN}&lang=ru"

        # Выполнение GET-запроса к OpenWeatherMap API
        response = requests.get(url)
        response.raise_for_status()

        # Получение данных о погоде из ответа в формате JSON
        weather_data = response.json()

        weather_description = weather_data["weather"][0]["description"]

        temperature = round(weather_data["main"]["temp"] - 273.15, 1)  # Кельвины в градусы Цельсия
        feels_like = round(weather_data["main"]["feels_like"] - 273.15, 1)

        pressure = weather_data["main"]["pressure"] * 0.75006375541921  # Гектопаскали в мм рт. ст.
        humidity = weather_data["main"]["humidity"]  # Влажность в процентах
        visibility = weather_data["visibility"]  # Видимость в метрах

        wind_speed = weather_data["wind"]["speed"]  # Скорость ветра в м/с
        wind_direction_deg = weather_data["wind"]["deg"]  # Направление ветра в градусах
        ...
        # Получение времени восхода и захода солнца в московском часовом поясе
...
def handler(event, context):
    ...
    try:
        ...
        if 'text' not in message_in:
            if 'voice' not in message_in:
                if 'location' not in message_in:
                    send_message('Могу обработать только текстовые и голосовые сообщения!', message_in)
                    return FUNC_RESPONSE
        ...
        if 'text' in message_in:
            if message_in['text'].lower() == '/START' or message_in['text'].lower() == '/HELP':
                message = """
